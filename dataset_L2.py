...
#       读取数据并划分数据集
# -------------------------------#
def data_load(path, test_path, size, is_train, is_sampling='down_sampler'):
    dicomlist = []    # 图像地址
    labels = []       # 图像标签
    train_img = []    # 训练集图像
    train_label = []  # 训练集标签
    ...
    if is_train or test_path == '':  # 训练模式或测试模式没有单独csv
        ...
        # train_img, train_label = smote.fit_resample(train_img, train_label)

        # balanced_images = []
        # balanced_labels = []
        #
        #
        # indices = np.where(train_label == '0')
        #
        #
        # selected_indices = random.sample(list(indices[0]), 8)
        #
        #
        # balanced_images.extend(train_img[selected_indices])
        # balanced_labels.extend(train_label[selected_indices])
        #
        # indices = np.where(train_label == '1')
        #
        # selected_indices = random.sample(list(indices[0]), 8)
        #
        # balanced_images.extend(train_img[selected_indices])
        ...
        # train_label = np.array(balanced_labels)
        # train_img = np.array(balanced_images)
...
#          加载数据集
# -------------------------------#
def get_dataset(path, test_path, size, batch_size, is_train, is_sampling):
